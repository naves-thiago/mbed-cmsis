...
EHCI_HOST_DATA_Type ehci_data[MAX_HOST_CONTROLLER] __BSS(USBRAM_SECTION);
...
{
	EHCI_REG(HostID)->USBSTS_H = 0xFFFFFFFF; 			/* clear all current interrupts */
	EHCI_REG(HostID)->PORTSC1_H &= ~(1<<12);			/* clear port power */
	EHCI_REG(HostID)->USBMODE_H =   (1<<0);				/* set USB mode reserve */
...
HCD_STATUS HcdRhPortReset(uint8_t HostID, uint8_t uPortNumber)
{
	...
	while (EHCI_REG(HostID)->PORTSC1_H & EHC_PORTSC_PortReset){}
...
{
	return EHCI_REG(HostID)->FRINDEX_H;
}
HCD_STATUS HcdGetDeviceSpeed( uint8_t HostID, uint8_t PortNumber, HCD_USB_SPEED* DeviceSpeed )
{
	if ( EHCI_REG(HostID)->PORTSC1_H & EHC_PORTSC_CurrentConnectStatus) /* If device is connected */
	{
		*DeviceSpeed = (HCD_USB_SPEED) ( (EHCI_REG(HostID)->PORTSC1_H & EHC_PORTSC_PortSpeed) >> 26 ); /* TODO magic number */
		return HCD_STATUS_OK;
	}else
	{
...
HCD_STATUS HcdClosePipe(uint32_t PipeHandle)
{
	...
	switch (XferType)
	{
		...
		case INTERRUPT_TRANSFER:
			...
			EHCI_REG(HostID)->USBCMD_H |= EHC_USBCMD_IntAsyncAdvanceDoorbell; /* DoorBell Handshake: Queue Head will only be free in AsyncAdvanceIsr */
		break;
...
static HCD_STATUS QueueITDs( uint8_t HostID, uint8_t IhdIdx, uint8_t* dataBuff, uint32_t xferLen )
{
	...
	FrameIdx = EHCI_REG(HostID)->FRINDEX_H >> 3;
...
static HCD_STATUS PipehandleParse(uint32_t Pipehandle, uint8_t* pHostID, HCD_TRANSFER_TYPE* XferType, uint8_t* pIdx)
{
	...
	if	(	pHandle->HostId >= MAX_HOST_CONTROLLER ||
...
{
	uint32_t IntStatus;
	IntStatus = EHCI_REG(HostID)->USBSTS_H & EHCI_REG(HostID)->USBINTR_H;
	...
	EHCI_REG(HostID)->USBSTS_H |= IntStatus;

	/* Process Interrupt Sources */
	if(IntStatus & EHC_USBSTS_PortChangeDetect)
	{
		uint32_t PortSC = EHCI_REG(HostID)->PORTSC1_H ;
		if (PortSC & EHC_PORTSC_ConnectStatusChange)
		{
			PortStatusChangeIsr(HostID, PortSC & EHC_PORTSC_CurrentConnectStatus);
			EHCI_REG(HostID)->PORTSC1_H |= EHC_PORTSC_ConnectStatusChange;	/* Clear PortSC Interrupt Status */
		}
		if (PortSC & EHC_PORTSC_PortEnableChange)
		{
			EHCI_REG(HostID)->PORTSC1_H |= EHC_PORTSC_PortEnableChange;		/* Clear PortSC Interrupt Status */
		}
		if (PortSC & EHC_PORTSC_OvercurrentChange)
		{
			EHCI_REG(HostID)->PORTSC1_H |= EHC_PORTSC_OvercurrentChange;	/* Clear PortSC Interrupt Status */
		}
		if (PortSC& EHC_PORTSC_ForcePortResume)
		{
			EHCI_REG(HostID)->PORTSC1_H |= EHC_PORTSC_ForcePortResume;		/* Clear PortSC Interrupt Status */
		}
	}
...
{
	EHCI_REG(HostID)->USBCMD_H |= EHC_USBCMD_RunStop;
	while (EHCI_REG(HostID)->USBSTS_H & EHC_USBSTS_HCHalted) {}
	return HCD_STATUS_OK;
}
...
{
	if (EHCI_REG(HostID)->USBSTS_H & EHC_USBSTS_HCHalted)
		EHciHostStop(HostID);

	EHCI_REG(HostID)->USBCMD_H |= EHC_USBCMD_HostReset;
	while( EHCI_REG(HostID)->USBCMD_H & EHC_USBCMD_HostReset ) {}
	
	/* Program the controller to be the USB host controller, this can only be done after Reset */
	LPC_USB0->USBMODE_H = USBMODE_HostController | USBMODE_VBusPowerSelect_High;
	return HCD_STATUS_OK;
}
...
//	memset(&ehci_data[HostID], 0, sizeof(EHCI_HOST_DATA_Type) );
	...
	/*---------- USBINT ----------*/
	EHCI_REG(HostID)->USBINTR_H &= ~EHC_USBINTR_ALL;	/* Disable All Interrupt */
	EHCI_REG(HostID)->USBSTS_H  &= ~EHC_USBINTR_ALL;	/* Clear All Interrupt Status */
	EHCI_REG(HostID)->USBINTR_H =	EHC_USBINTR_UsbAsyncEnable | EHC_USBINTR_UsbPeriodEnable | /* Enable necessary interrupt source: Async Advance, System Error, Port Change, USB Error, USB Int */
	...
	EHCI_REG(HostID)->ASYNCLISTADDR = (uint32_t) HcdAsyncHead(HostID);
...
static __INLINE void DisableSchedule( uint8_t HostID, uint8_t isPeriod )
{
	...
	if (EHCI_REG(HostID)->USBSTS_H & statusMask)
	{
		EHCI_REG(HostID)->USBCMD_H &= ~cmdMask;
		while (EHCI_REG(HostID)->USBSTS_H & statusMask){} /* TODO Should have time-out */
	}
}
