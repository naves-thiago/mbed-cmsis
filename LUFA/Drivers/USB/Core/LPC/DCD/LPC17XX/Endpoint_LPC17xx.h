...
#define __ENDPOINT_LPC17XX_H__
	...
	/* Private Interface - For use in library only: */
		...
		/* Macros: */
			#define ENDPOINT_DETAILS_MAXEP		16	/* Maximum of supported endpoint */
			#define USED_PHYSICAL_ENDPOINTS		12 	/* This macro effect memory size of the DCD */
		...
		/* Inline Functions: */
			...
			static inline bool Endpoint_IsINReady(void)
			{
				if (endpointselected==ENDPOINT_CONTROLEP)
				{
					...
					isInReady = SIE_ReadCommandData(DAT_SEL_EP(ENDPOINT_CONTROLEP+1)) & EP_SEL_F ? false : true ;       /* Select Endpoint Status */
			...
			static inline bool Endpoint_IsOUTReceived(void)
			{
				if (endpointselected==ENDPOINT_CONTROLEP)
				{
					...
					isOutReceived = SIE_ReadCommandData(DAT_SEL_EP(ENDPOINT_CONTROLEP)) & EP_SEL_F ? true : false ;       /* Select Endpoint Status */
					HAL17XX_EnableUSBInterrupt();
			...
			static inline void Endpoint_ClearOUT(void)
			{
				...
				{
// 					WrCmd(CMD_SEL_EP(endpointhandle[endpointselected]));
// 					WrCmd(CMD_CLR_BUF);
				}else
				{
